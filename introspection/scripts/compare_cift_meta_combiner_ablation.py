...
@dataclass(frozen=True)
class CompareCiftMetaCombinerAblationScriptConfig:
    ...
    combiner_rules: tuple[CiftMetaCombinerRule, ...]
    risk_label: str
    ridge: float
    fold_count: int
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run CIFT meta-head constrained-combiner ablations.")
    parser.add_argument(
        "--dataset-artifact",
        required=False,
        action="append",
...
def _parse_combiner_rules(values: Sequence[str] | None) -> tuple[CiftMetaCombinerRule, ...]:
    ...
    if len(set(raw_values)) != len(raw_values):
        raise ValueError("Combiner rules must be unique.")
    return tuple(cast(CiftMetaCombinerRule, value) for value in raw_values)


def _parse_args(argv: Sequence[str]) -> CompareCiftMetaCombinerAblationScriptConfig:
    namespace = _build_parser().parse_args(argv)
    return CompareCiftMetaCombinerAblationScriptConfig(
        ...
        combiner_rules=_parse_combiner_rules(namespace.combiner_rule),
...
def _binary_task_config(config: CompareCiftMetaCombinerAblationScriptConfig) -> BinaryTaskConfig:
...
def _load_datasets(config: CompareCiftMetaCombinerAblationScriptConfig) -> tuple[CiftMetaCombinerDataset, ...]:
    return tuple(
        CiftMetaCombinerDataset(
...
def _source_feature_keys(dataset: CiftMetaCombinerDataset) -> tuple[str, ...]:
    return (
...
def _variants(
    ...
    config: CompareCiftMetaCombinerAblationScriptConfig,
) -> tuple[CiftMetaCombinerVariant, ...]:
    if len(datasets) == 0:
        raise ValueError("At least one dataset is required to resolve CIFT combiner source features.")
    source_feature_keys = _source_feature_keys(datasets[0])
    return tuple(
        CiftMetaCombinerVariant(
            variant_id=combiner_rule,
            feature_name=f"cift_meta_combiner_{combiner_rule}",
        ...
        for combiner_rule in config.combiner_rules
    )


def run_comparison(config: CompareCiftMetaCombinerAblationScriptConfig) -> None:
    ...
    write_cift_meta_combiner_ablation_json(config.output_json_path, report)
    write_cift_meta_combiner_ablation_markdown(config.output_markdown_path, report)

    print(f"Wrote CIFT meta-head combiner ablation report to {config.output_json_path}")
    print(f"Wrote CIFT meta-head combiner ablation summary to {config.output_markdown_path}")
    print(
