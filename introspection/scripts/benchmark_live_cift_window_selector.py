...
from aegis_introspection.cift_live_selector_benchmark import (  # noqa: E402
    CiftLiveWindowSelectorBenchmarkConfig,
    ModelDTypeName,
    run_cift_live_window_selector_benchmark,
)
from aegis_introspection.model_loader import parse_model_dtype  # noqa: E402
...
def run_cli(config: BenchmarkLiveCiftWindowSelectorCliConfig) -> None:
    report = run_cift_live_window_selector_benchmark(_benchmark_config(config))
    print(f"Wrote live CIFT window-selector benchmark JSON to {config.output_json_path}")
    print(f"Wrote live CIFT window-selector benchmark summary to {config.output_markdown_path}")
    print(f"Requests: {report.request_count}")
    print(f"Window families: {report.window_family_counts}")
