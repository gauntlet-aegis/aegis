...
@dataclass(frozen=True)
class CompareCiftMetaFamilyInteractionsScriptConfig:
    ...
    interaction_rules: tuple[CiftMetaFamilyInteractionRule, ...]
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run CIFT meta-head source-family interaction comparisons.")
    parser.add_argument(
        "--dataset-artifact",
        required=False,
        action="append",
    ...
    parser.add_argument(
        "--output-json",
        required=False,
        default=str(INTROSPECTION_ROOT / "data" / "reports" / "cift_meta_family_interactions_v1.json"),
    )
    parser.add_argument(
        "--output-md",
        required=False,
        default=str(INTROSPECTION_ROOT / "data" / "reports" / "cift_meta_family_interactions_v1_summary.md"),
    )
    ...
    parser.add_argument(
        ...
        required=False,
        action="append",
        choices=("raw_scores", "family_means", "family_mean_gaps"),
        help="Meta-head source-family interaction rule. Defaults to all supported rules.",
    )
...
def _parse_interaction_rules(values: Sequence[str] | None) -> tuple[CiftMetaFamilyInteractionRule, ...]:
    ...
    if len(raw_values) == 0:
        raise ValueError("At least one family interaction rule is required.")
    if len(set(raw_values)) != len(raw_values):
        raise ValueError("Family interaction rules must be unique.")
    return tuple(cast(CiftMetaFamilyInteractionRule, value) for value in raw_values)


def _parse_args(argv: Sequence[str]) -> CompareCiftMetaFamilyInteractionsScriptConfig:
    namespace = _build_parser().parse_args(argv)
    return CompareCiftMetaFamilyInteractionsScriptConfig(
        ...
        interaction_rules=_parse_interaction_rules(namespace.interaction_rule),
...
def _binary_task_config(config: CompareCiftMetaFamilyInteractionsScriptConfig) -> BinaryTaskConfig:
...
def _load_datasets(
    config: CompareCiftMetaFamilyInteractionsScriptConfig,
) -> tuple[CiftMetaFamilyInteractionDataset, ...]:
    return tuple(
        CiftMetaFamilyInteractionDataset(
...
def _final_token_source_feature_keys(dataset: CiftMetaFamilyInteractionDataset) -> tuple[str, ...]:
    return last_quarter_readout_feature_keys(dataset.artifact, "final_token")


def _mean_pool_source_feature_keys(dataset: CiftMetaFamilyInteractionDataset) -> tuple[str, ...]:
    return last_quarter_readout_feature_keys(dataset.artifact, "mean_pool")


def _variants(
    datasets: tuple[CiftMetaFamilyInteractionDataset, ...],
    config: CompareCiftMetaFamilyInteractionsScriptConfig,
) -> tuple[CiftMetaFamilyInteractionVariant, ...]:
    if len(datasets) == 0:
        raise ValueError("At least one dataset is required to resolve CIFT family-interaction source features.")
    reference_dataset = datasets[0]
    final_token_source_feature_keys = _final_token_source_feature_keys(reference_dataset)
    mean_pool_source_feature_keys = _mean_pool_source_feature_keys(reference_dataset)
    return tuple(
        CiftMetaFamilyInteractionVariant(
            variant_id=interaction_rule,
            feature_name=f"cift_meta_family_interactions_{interaction_rule}",
            final_token_source_feature_keys=final_token_source_feature_keys,
            mean_pool_source_feature_keys=mean_pool_source_feature_keys,
        ...
        for interaction_rule in config.interaction_rules
    )


def run_comparison(config: CompareCiftMetaFamilyInteractionsScriptConfig) -> None:
    ...
    write_cift_meta_family_interactions_json(config.output_json_path, report)
    write_cift_meta_family_interactions_markdown(config.output_markdown_path, report)

    print(f"Wrote CIFT meta-head family-interactions report to {config.output_json_path}")
    print(f"Wrote CIFT meta-head family-interactions summary to {config.output_markdown_path}")
    print(
